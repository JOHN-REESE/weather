...
from geopy.exc import GeocoderTimedOut, GeocoderUnavailable2
...
def get_coordinates_for_city(city_name: str) -> Union[Dict[str, float], None]:
    """
    ...
    Returns:
        Union[Dict[str, float], None]: A dictionary containing 'latitude' and 'longitude',
              or None if the location cannot be determined.
    """
    try:
        geolocator = Nominatim(user_agent="city_weather_app/1.1")
        ...
        print(f"An unexpected error occurred while geocoding {city_name}: {e}")
        return None
...
if __name__ == '__main__':
    ...
    print("Enter '1' to get weather for your current location.")
    print("Enter '2' to get weather for a specific city.")
    ...
    if choice == '1':
        ...
        city_name_input = input("Enter the city name: ")
        print(f"Fetching coordinates for {city_name_input}...")
        coordinates = get_coordinates_for_city(city_name_input)

        if coordinates:
            latitude = coordinates.get("latitude")
            longitude = coordinates.get("longitude")
            print(f"Fetching weather for {city_name_input} ({latitude}, {longitude})...")
            weather_data = get_weather(latitude, longitude)

            if weather_data:
                temperature = weather_data.get("temperature")
                description = weather_data.get("weather_description")
                print(f"The current weather in {city_name_input} is: {temperature}°C, {description}.")
            else:
                print(f"Could not retrieve weather information for {city_name_input}.")
        else:
            print(f"Could not find coordinates for {city_name_input}.")
    else:
